...
const loadData = async () => {
  PostData(UrlConstants.apiGetDivisionWiseSummaryRpt, gridConfig.value, "", (response: any) => {
    const allRows = response;
    const lastRow = allRows.find((row: any) => row.Division === "Grand Total");
    const otherRows = allRows.filter((row: any) => row.Division !== "Grand Total");

    resultData.value = otherRows;
    footerRow.value = lastRow || {};

    const keys = Object.keys(otherRows[0] || {});
    columnList.value = keys.filter(k => k !== 'Division');
  }, null);
};
...
const exportData = (exportType: any) => {
    ...
    ExportData(UrlConstants.apiGetDivisionWiseSummaryRpt,Postdata, ("Division_Summary_"+(moment(Date.now()).utcOffset(0, true).format("YY_MM_DD_hhmmss"))),"",(exportType == 1)?"xlsx":"pdf", (data: any) => {        
